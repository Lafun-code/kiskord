// Based on the implementation from Jitsi Meet
// https://github.com/jitsi/jitsi-meet/blob/master/react/features/stream-effects/rnnoise/

// We inline the rnnoise module loader here to avoid 'importScripts' issues in ES modules.
// This matches the user's request to fix "importScripts is not defined".
...
const createModule = createRNNWasmModule;
...
class RNNoiseWorklet extends AudioWorkletProcessor {
    constructor() {
        ...
        // State
        this._denoiseState = 0;
        this._pcmInput = 0;
        this._heapFloat32 = null;
        this._module = null;
        ...
        this.port.onmessage = (event) => {
            if (event.data.type === 'init') {
                ...
                this._isEnabled = event.data.value;
            }
        };
    }
    ...
    _initWasm(wasmBytes) {
        try {
             const moduleConfig = {};
             if (wasmBytes) {
                 moduleConfig.wasmBinary = wasmBytes;
             }
             
             // Inlined createModule usage
             createModule(moduleConfig).then(Module => {
                 this._module = Module;
                 this._denoiseState = Module._rnnoise_create(null);
                 this._pcmInput = Module._malloc(FRAME_SIZE * 4);
                 this._heapFloat32 = Module.HEAPF32;
    ...
    process(inputs, outputs, parameters) {
        ...
        // If disabled or not ready, pass through
        if (!this._isEnabled || !this._denoiseState || !this._module) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            // Add to input buffer
            this._inputBuffer[this._inputBufferIndex++] = inputChannel[i];
            
            // When buffer is full, process it
            if (this._inputBufferIndex === FRAME_SIZE) {
                this._processFrame();
                this._inputBufferIndex = 0;
            }
            
            // Read from output buffer (circularly or just next sample)
            if (this._outputBufferIndex >= FRAME_SIZE) {
                 this._outputBufferIndex = 0;
            }
            
            outputChannel[i] = this._outputBuffer[this._outputBufferIndex++];
        }
    ...
    _processFrame() {
        if (!this._heapFloat32 || !this._module) return;

        // Copy input to WASM memory
        this._heapFloat32.set(this._inputBuffer, this._pcmInput >> 2);
        
        // Process
        this._module._rnnoise_process_frame(this._denoiseState, this._pcmInput, this._pcmInput);
        
        // Copy output from WASM memory
        const outputData = this._heapFloat32.subarray(
            this._pcmInput >> 2,
            (this._pcmInput >> 2) + FRAME_SIZE
        );
        
        this._outputBuffer.set(outputData);
    }
}
